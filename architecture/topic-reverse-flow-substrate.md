...
slug: topic-reverse-flow-substrate
...
cites: []
paired_with: topic-reverse-flow-substrate.es.md
---
...
The planned reverse flows depend on [[topic-single-boundary-compute-discipline]] — all marketplace and ad exchange traffic is intended to route through the Doorman, so the same audit boundary that captures inference also captures monetization events. They depend on [[topic-customer-owned-graph-ip]] — the inventory is the customer's intellectual property, and reverse flows are the mechanism through which the customer may realize commercial value from that property. And they depend on [[topic-direct-payment-settlement]] — payment flows directly from buyer to the customer-tenant, with Foundry taking a transaction fee at settlement.

## See Also

- [[topic-customer-owned-graph-ip]] — the ownership principle that makes reverse flows a customer-controlled decision
- [[topic-direct-payment-settlement]] — the planned payment mechanism for marketplace and ad exchange transactions
- [[topic-vertical-seed-packs-marketplace]] — planned packs as first-class marketplace inventory
- [[topic-single-boundary-compute-discipline]] — the Doorman boundary that governs outbound as well as inbound flows
