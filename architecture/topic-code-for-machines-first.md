...
slug: topic-code-for-machines-first
...
cites: []
paired_with: topic-code-for-machines-first.es.md
---
...
Inter-service communication uses MCP (see [[topic-mcp-substrate-protocol]]). The audit ledger uses JSONL with schema versioning. Seed taxonomies use JSON. Service configuration uses TOML or YAML. Conventions and documentation use Markdown with structured frontmatter. Per-tenant configuration uses YAML.
...
**AI-native composition.** The substrate is consumable by AI agents — task sessions, customer-built agents, partner integrations — without a retrofit step. This is what makes the [[topic-knowledge-graph-grounded-apprenticeship]] pattern possible: the graph is already machine-readable; the Doorman can query it programmatically at inference time.

**Migration ease.** Data export is a routine machine operation producing an open-format bundle (see [[topic-substrate-without-inference-base-case]]). There is no migration project involving vendor cooperation, because the data was never locked in a vendor-specific format.
...
This discipline is the structural claim that [[topic-mcp-substrate-protocol]] realizes at the wire level. Every service exposes its contract through MCP `describe`, which returns a machine-readable tool catalog. Human-facing surfaces consume this catalog the same way any automated client would.

## See Also

- [[topic-mcp-substrate-protocol]] — the structural realization of machine-first service contracts via MCP
- [[topic-knowledge-graph-grounded-apprenticeship]] — graph queries are machine-first MCP tool calls
- [[topic-substrate-without-inference-base-case]] — export bundles are machine-readable open formats
