...
slug: topic-customer-owned-graph-ip
...
cites: []
paired_with: topic-customer-owned-graph-ip.es.md
---
...
**Transfer of ownership.** The customer may sell, gift, or transfer their Totebox and export bundle without Foundry's involvement. This is addressed in more detail in [[topic-substrate-without-inference-base-case]].
...
Foundry's approach inverts this. The customer's data is shaped by their own seed taxonomy (see [[topic-seed-taxonomy-as-smb-bootstrap]]). Export is a single-command operation producing an open-format bundle. The customer's access to their data does not depend on Foundry's servers remaining operational.

The economic complement is the transaction-fee pricing model (see [[topic-direct-payment-settlement]]): the customer pays Foundry a fee only when they earn from their data, not a recurring subscription for access to their own records.

## Relationship to reverse flows

The [[topic-reverse-flow-substrate]] describes the mechanisms by which a customer's accumulated graph may become a saleable asset — patterns such as a data marketplace or an ad exchange. These reverse flows are opt-in per tenant and require explicit operator consent at each transaction. The customer's ownership is the foundation that makes monetization a customer decision: the customer is selling or licensing their own intellectual property, with Foundry facilitating the transaction for a fee.
...
- [[topic-seed-taxonomy-as-smb-bootstrap]] — the customer's customized seed taxonomy is part of their intellectual property
- [[topic-reverse-flow-substrate]] — planned mechanisms for monetizing customer-owned graph assets
- [[topic-direct-payment-settlement]] — the payment model that makes customer ownership commercially meaningful
- [[topic-substrate-without-inference-base-case]] — the transfer-of-ownership flow that makes the ownership right exercisable
