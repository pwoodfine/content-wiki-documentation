...
slug: topic-knowledge-graph-grounded-apprenticeship
...
cites: []
paired_with: topic-knowledge-graph-grounded-apprenticeship.es.md
---
...
Knowledge-graph-grounded apprenticeship depends on the [[topic-single-boundary-compute-discipline]]. If inference can bypass the Doorman, it bypasses graph grounding. An ungrounded inference call produces no graph context field in the training tuple, no citation rate measurement, and no proposed graph mutation. The two claims compose as a structural dependency: without single-boundary enforcement, graph-grounded apprenticeship cannot be guaranteed.

## See Also

- [[topic-single-boundary-compute-discipline]] — structural prerequisite; grounding happens at the Doorman boundary
- [[topic-seed-taxonomy-as-smb-bootstrap]] — the per-tenant taxonomy that seeds the knowledge graph used for grounding
- [[topic-mcp-substrate-protocol]] — the MCP tools (`graph_query`, `graph_mutate`) through which the Doorman interacts with `service-content`
